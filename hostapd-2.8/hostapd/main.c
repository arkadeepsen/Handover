...
#ifdef CONFIG_OF_SWITCH_AP
	int resp = controller_conn_init(interfaces.iface[0]->bss[0]);
	if(resp == 1)
		goto out;
	pthread_t thread;
	pthread_t thread_rssi;
	int rc;
	rc = pthread_create(&thread, NULL, controller_run, NULL);
	if (rc)
	{
		printf("ERROR; return code from pthread_create() is %d\n", rc);
//		exit(-1);
	}
	rc = pthread_create(&thread_rssi, NULL, data_frames_rssi, NULL);
	if (rc)
	{
		printf("ERROR; return code from pthread_create() is %d\n", rc);
//		exit(-1);
