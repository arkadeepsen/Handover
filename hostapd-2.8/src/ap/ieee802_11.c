...
#ifdef CONFIG_OF_SWITCH_AP
#include <pthread.h>
#include "ofSwitchAP/controller_interface.h"
#endif /* CONFIG_OF_SWITCH_AP */
...
#ifdef CONFIG_OF_SWITCH_AP
	struct infoSTA *info = lookupSenderSTA(mgmt->sa);
	if(!info || info->status != SELECTED)
	{
		return;
	}
#endif /* CONFIG_OF_SWITCH_AP */
...
#ifdef CONFIG_OF_SWITCH_AP
		changeStatus(info, AUTHENTICATED);
		pthread_t thread;
		int rc;
		struct mgmt_handle *handle = malloc(sizeof(struct mgmt_handle));
		handle->mgmt = malloc(len);
		os_memcpy(handle->mgmt, mgmt, len);
		handle->len = len;
		rc = pthread_create(&thread, NULL, process_sta_msg, (void *)handle);
		if (rc)
		{
...
#ifdef CONFIG_OF_SWITCH_AP
	struct infoSTA *info = lookupSenderSTA(mgmt->sa);
	if(!info || info->status != AUTHENTICATED)
	{
		return;
	}
#endif /* CONFIG_OF_SWITCH_AP */
			    ...
			    const struct ieee80211_mgmt *mgmt, size_t len)
{
	...
	if (sta->timeout_next == STA_NULLFUNC ||
		...
		eloop_register_timeout(AP_DEAUTH_DELAY, 0, ap_handle_timer,
				       hapd, sta);
		u8 *addr;
		addr = calloc(1, ETH_ALEN);
		os_memcpy(addr, sta->addr, ETH_ALEN);
		eloop_register_timeout(AP_DEAUTH_DELAY, 0, notify_controller_disassoc,
						       NULL, addr);
	}
...
#ifdef CONFIG_OF_SWITCH_AP
	struct infoSTA *info = lookupSenderSTA(mgmt->da);
	if(!info || info->status != AUTHENTICATED)
	{
		return;
	}
...
#ifdef CONFIG_OF_SWITCH_AP
	struct infoSTA *info = lookupSenderSTA(mgmt->da);
	if(!info || info->status != NOT_AUTHENTICATED)
	{
		return;
	}
#endif /* CONFIG_OF_SWITCH_AP */
...
#ifdef CONFIG_OF_SWITCH_AP
	struct infoSTA *info = lookupSenderSTA(addr);
	if(!info || info->status != ASSOCIATED)
	{
		return;
	}
...
#ifdef CONFIG_OF_SWITCH_AP
	struct infoSTA *info = lookupSenderSTA(dst);
	if(!info || info->status != ASSOCIATED)
	{
		return;
	}
...
#ifdef CONFIG_OF_SWITCH_AP
	struct infoSTA *info = lookupSenderSTA(src);
	if(!info || info->status != ASSOCIATED)
	{
		return;
	}
...
void add_STA(struct hostapd_data *hapd, struct infoSTA *infoSta)
{
	u16 auth_alg;
	struct sta_info *sta = NULL;
	int i;
...
//	struct ieee80211_vht_capabilities vht_cap;
	...
	auth_alg = infoSta->auth_alg;
	wpa_printf(MSG_DEBUG, "add: STA=" MACSTR " auth_alg=%d",
		   MAC2STR(infoSta->address), auth_alg);
	...
	sta = ap_get_sta(hapd, infoSta->address);
	if (!sta) {
#ifdef CONFIG_MESH
		if (hapd->conf->mesh & MESH_ENABLED) {
			/* if the mesh peer is not available, we don't do auth.
		...
		sta = ap_sta_add(hapd, infoSta->address);
		if (!sta) {
			return;
		}
	}
...
//	hostapd_free_psk_list(sta->psk);
	...
	sta->capability = infoSta->capability;
	sta->listen_interval = infoSta->listen_interval;

	os_memcpy(sta->supported_rates, infoSta->supported_rates, infoSta->supported_rates_len);
	sta->supported_rates_len = infoSta->supported_rates_len;
...
void remove_STA(struct hostapd_data *hapd, struct infoSTA *infoSta)
{
	struct sta_info *sta;

	sta = ap_get_sta(hapd, infoSta->address);
	if (sta == NULL) {
		wpa_printf(MSG_INFO, "Station " MACSTR " trying to remove, but it is not added",
			   MAC2STR(infoSta->address));
		return;
	}
...
void notify_controller_disassoc(void *eloop_ctx, void *timeout_ctx)
{
	u8 *addr = timeout_ctx;
#ifdef CONFIG_OF_SWITCH_AP
	struct infoSTA *info = lookupSenderSTA(addr);
	...
	pthread_t thread;
	int rc;
	rc = pthread_create(&thread, NULL, handle_deauth_conn, (void *)addr);
	if (rc)
	{
		printf("ERROR; return code from pthread_create() is %d\n", rc);
//		exit(-1);
